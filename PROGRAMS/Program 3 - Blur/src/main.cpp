...
#define SSBOSIZE 4096*4

class ssbo_data
{
    public:
        ivec2 pixels[SSBOSIZE];
};
...
public:
	...
	bool noisy_blur = false;

	ssbo_data blurpixels;
	GLuint CScollect, CSblur, CSblur_second, SSBOid;
	...
	void keyCallback(GLFWwindow *window, int key, int scancode, int action, int mods)
	{
		...
		if (key == GLFW_KEY_N && action == GLFW_PRESS)		noisy_blur = !noisy_blur;
...
void compute_blur(GLuint shader)
	{
        glUseProgram(shader);
        GLuint block_index = 0;
        block_index = glGetProgramResourceIndex(shader, GL_SHADER_STORAGE_BLOCK, "shader_data");
        GLuint ssbo_binding_point_index = 0;
        glShaderStorageBlockBinding(shader, block_index, ssbo_binding_point_index);
        glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 0, SSBOid);
		glBindImageTexture(0, FBOmask, 0, GL_FALSE, 0, GL_READ_WRITE, GL_RGBA8);
	...
	void initGeom(const std::string& resourceDirectory)
	{
		...
		// Attach Shadow Map depth texture to Shadow Map FBO
		{
			...
			case GL_FRAMEBUFFER_COMPLETE:
				...
				break;
			default:
				cout << "status framebuffer: very bad!!!!!!!!!!!!!!!!!!!!!!!!!" << std::endl;
			}
			glBindFramebuffer(GL_FRAMEBUFFER, 0);
		}
		...
		CSblur_second = init_computeshader(resourceDirectory + "/compute_blur_second.glsl");
...
int main(int argc, char **argv)
{
	...
	while (! glfwWindowShouldClose(windowManager->getHandle()))
	{
		...
		application->compute();
		application->compute_blur(application->CSblur);
		if (application->noisy_blur) {
			application->compute_blur(application->CSblur_second);
		}
