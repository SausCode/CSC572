...
public:
	...
	// Our shader program
	std::shared_ptr<Program> geometry_prog, ssao_prog, shading_prog;
	...
	void init(const std::string& resourceDirectory)
	{
		...
		shading_prog = make_shared<Program>();
		shading_prog->setVerbose(true);
		shading_prog->setShaderNames(resourceDirectory + "/blur_vert.glsl", resourceDirectory + "/blur_frag.glsl");
		if (!shading_prog->init())
		{
			std::cerr << "One or more shaders failed to compile... exiting!" << std::endl;
			exit(1);
		}
		shading_prog->init();
		shading_prog->addUniform("light_color");
		shading_prog->addUniform("light_pos");
		shading_prog->addAttribute("vertPos");
		shading_prog->addAttribute("vertTex");
	}
	...
	void initGeom(const std::string& resourceDirectory)
	{
		...
		Tex1Loc = glGetUniformLocation(shading_prog->pid, "ambient_occlusion");
		Tex2Loc = glGetUniformLocation(shading_prog->pid, "gPosition");
		Tex3Loc = glGetUniformLocation(shading_prog->pid, "gNormal");
		Tex4Loc = glGetUniformLocation(shading_prog->pid, "gColor");
	...
	void render_to_screen(){
		...
		GLint loc = glGetUniformLocation(shading_prog->pid, "light_color");
		if (loc != -1)
		{
			if (!party_time) {
				light_color = vec3(1);
			}
			glUniform3fv(shading_prog->getUniform("light_color"), 1, &light_color.x);
		}

		loc = glGetUniformLocation(shading_prog->pid, "light_pos");
		if (loc != -1)
		{
			glUniform3fv(shading_prog->getUniform("light_pos"), 1, &light_pos.x);
		}
		glBindVertexArray(VertexArrayIDBox);
	...
	void make_ssaoKernel() {
		for (unsigned int i = 0; i < 64; ++i){
			...
			// Make z "static_cast <float> (rand()) / static_cast <float> (RAND_MAX) * 2.0 - 1.0," 
			// if you want sphere and not dome samples
			sample = glm::normalize(sample);
			sample *= static_cast <float> (rand()) / static_cast <float> (RAND_MAX);
			float scale = (float)i / 64.0;
			// Lerp to concentrate samples in the middle of the dome
	...
	void render_to_texture(){
		...
		sponza->draw(geometry_prog, false);
